...
class DraughtRePlay:
    ...
    def incr_tag(self, tag, cntl = False):
        ...
        if pnt == game.half_ply_length(): # at the end ?
            if len(pnts) > 2:             # variant ?
                pnts = pnts[:-1]          # back to previous level
                self.main.Games.pop()     # remove the previous game
                self.main.ActiveGame = games[-2]
    ...
    def compose_tag(self, pnts):
        self.main.LeadingTag = str(pnts[0]).zfill(3) + '_'
        for k in range(1, len(pnts)-1):
            self.main.LeadingTag += str(pnts[k]).zfill(3) + '_'
    ...
    def click(self, tag):
        ...
        if self.main.ActiveTag <> '':
            self.main.panel.notation.tag_config(self.main.ActiveTag, background = "#e8e8e8")
        self.main.panel.notation.tag_config(tag, background = "yellow")
        #print self.line_nr(self.main.panel.notation.tag_ranges(tag)[0])
        self.main.panel.notation.yview_scroll(self.line_nr(self.main.panel.notation.tag_ranges(tag)[0])-1, UNITS)
...
#                game.ActivePanel = new
                game.ActivePanel = prev_game.ActivePanel
