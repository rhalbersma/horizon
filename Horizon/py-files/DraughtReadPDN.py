...
#          print "half_ply_length = ", game.half_ply_length()
    ...
    def GetSimpleKeywordLine(self, start, game):
        ...
        elif word == 'WhiteTime': pass
        elif word == 'BlackTime': pass
        elif word == 'DamboTopLine': pass
        elif word == 'DamboBottomLine': pass
        elif word == 'FEN':
            temp = self.GetBeginPosition(text_start, game)
            if temp == 0: return 0
        else:
            print "Found Keyword: ", word
            return 0
        if self.debug: print "SimpleKeywordLine found"
        return cp
    ...
    def GetCloseRoundBracket(self, start):
        cp = self.LogProgress("Get Close Round Bracket", start)
        cp = self.SkipWhiteSpace(start)
        if self.s[cp] == ')' or self.s[cp] == ']':
            cp = self.Incr(cp)
            cp = self.SkipWhiteSpace(cp)
    ...
    def GetWhiteVariant(self, start, game, half_ply):
        ...
        if cp == 0: return 0
        new_game = GameRecord(self.main, self.main.panel, self.main.panel, game)
    ...
    def GetTerminator(self, start, game):
        ...
        cp = self.SkipWhiteSpace(cp)
        result = self.s[cp:cp+3]  # maybe
        if self.s[cp:cp+3] == '1-0' or \
           self.s[cp:cp+3] == '0-1' or \
           self.s[cp:cp+3] == '2-0' or \
           self.s[cp:cp+3] == '0-2' or \
           self.s[cp:cp+3] == '1-1':
          game.Result = result
          return cp + 3
        if self.s[cp:cp+3] == '0-0': return cp+3
        if self.s[cp:cp+7] == '1/2-1/2':
