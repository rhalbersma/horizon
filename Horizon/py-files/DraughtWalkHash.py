...
class DraughtWalkHash:
    ...
    def show_follow_pressed(self):
        #print self.Positions[-1].board
        selection = self.panel.all_moves.curselection()
        if selection==():
            move_nr = 0
        else:
            move_nr = int(selection[0])
        #print move_nr
        self.main.play.engine.send_follow_up_move(self.Positions[-1].board, move_nr)
        return
...
# best move index (2)
    ...
    def get_hash_node(self,  msg,  p):
        ...
        if st == " 0" : new_rec.move_status = "min node free move"
        elif st == " 1": new_rec.move_status = "max node free move"
        elif st == "10" : new_rec.move_status = "min captured"
        elif st == "11": new_rec.move_status = "max captured"
        elif st == "12": new_rec.move_status = "re-captured"
        else: new_rec.move_status = "Unknown"
        new_rec.depth = msg[tail_start+6:tail_start+9]
        #print msg[tail_start:]
