...
class GameRecord:
    ...
    def __init__(self, main, active_panel, support_panel,  prev_game='', notation = None):
        ...
        if notation:
            ...
            self.notation = self.panel.notation
        ...
        self.main.Games.append(self)
        self.OwnGameIndex = len(self.main.Games) - 1   # to find the next game on the lowest level.
        return
    ...
    def half_ply_length(self):
        ...
        if not self.Moves[len(self.Moves) - 1].BlackMove : k = k - 1
        return k
    ...
    def refresh_game_info(self):
        ...
        if self.panel.Result: 
            if self.Result <> '': self.panel.Result.set(str(self.Result))
        if self.panel.Event: 
 ...
 #       if self.main.panel.is_play_mode():
    ...
    def get_next_board_layout(self):
        ...
        if valid_move == None: return None
        self.do_move(valid_move, False)
    ...
    def write_moves(self):
        ...
        self.notation.config(state=DISABLED)
        return

    def init_moves_display(self):
        self.LastCharCount = 0                     # to be able to insert a \n before the 80th char
        self.notation.config(state=NORMAL)
        self.notation.delete(1.0, END)
        return
    ...
    def put_moves_display(self, format = True):
        ...
        for j in range(0,len(self.Moves)):
            if self.LastCharCount >= 65:
                self.notation.insert(END, '\n')
                self.LastCharCount = 0
            if self.Moves[j].Ply_nr <> '':
                self.notation.insert(END, repr(self.Moves[j].Ply_nr).rjust(2) + ".")    
            self.LastCharCount = self.LastCharCount + 3
            if self.Moves[j].WhiteMove:
                ...
                if self.Moves[j].WhiteMove.Comment <> '':
                    ...
                    self.LastCharCount = self.LastCharCount + len(self.Moves[j].WhiteMove.Comment) + 3
                    
                    


                # we have the white move in s
                tag = str(self.OwnGameIndex).zfill(3) + '_' + str(self.HalfPlyDisplayPointer).zfill(3)
                self.notation.insert(END, s, tag)
                self.notation.tag_bind(tag, "<Button-1>", ClickMoveTag(self.main.re_play, tag))
                self.notation.insert(END, ' ')
                self.LastCharCount = self.LastCharCount + len(s) + 1
                
                if self.Moves[j].WhiteMove.Variant <> '':
                    self.notation.insert(END, '(')
                    self.LastCharCount = self.LastCharCount + 1
                    self.Moves[j].WhiteMove.Variant.LastCharCount = self.LastCharCount 
                    self.Moves[j].WhiteMove.Variant.put_moves_display(False)
                    self.notation.insert(END, ') ')
                    self.LastCharCount = self.Moves[j].WhiteMove.Variant.LastCharCount + 2
            ...
            if self.Moves[j].BlackMove:
                ...
                if self.Moves[j].BlackMove.Comment <> '':
                    ...
                    self.LastCharCount = self.LastCharCount + len(self.Moves[j].BlackMove.Comment) + 3
                ...
                if self.Moves[j].BlackMove.Variant <> '':
                    self.notation.insert(END, '(')
                    self.LastCharCount = self.LastCharCount + 1
                    self.Moves[j].BlackMove.Variant.LastCharCount = self.LastCharCount 
                    self.Moves[j].BlackMove.Variant.put_moves_display(False)
                    self.notation.insert(END, ') ')
                    self.LastCharCount = self.Moves[j].BlackMove.Variant.LastCharCount + 2

                self.HalfPlyDisplayPointer = self.HalfPlyDisplayPointer + 1
        
        tag = str(self.OwnGameIndex).zfill(3) + '_' + str(self.HalfPlyDisplayPointer).zfill(3)
        self.notation.insert(END, '#', tag)
        self.notation.tag_bind(tag, "<Button-1>", ClickMoveTag(self.main.re_play, tag))
        return
    ...
    def remove_from_tail(self):
        if len(self.Moves) > 0:
            if not self.Moves[len(self.Moves) - 1].BlackMove:
    ...
    def get_half_ply_length(self):
        if len(self.Moves) > 0:
            ...
            if not self.Moves[0].WhiteMove: calc_pnt -=  1
            if not self.Moves[-1].BlackMove: calc_pnt -= 1
            return calc_pnt
        else: return 0
